...
1. **`organizations` gains a `code`** — globally unique, immutable, `^[a-z0-9][a-z0-9-]{1,30}$`, assigned at organization creation. *(Amended: the pattern originally required a leading letter. It was relaxed to allow a leading digit when the provider's own organization (`--operator-org`), named `816tech`, could not be migrated, and nothing downstream — NATS subject tokens, leaf JetStream domains, KV bucket names, RFC 1123 hostname labels — justified the restriction. Leaf domains were `edge-<code>` at the time; they are now the bare Thing code.)* This is the **only** globally unique identifier in the ecosystem. Everything below it stays unique within its organization, which is already enforced.
...
Every sticker carries its payload as human-readable text alongside the symbol. Not for decoration — for the scratched, greasy, badly-lit label that will not scan, where a tech needs to read the code aloud or type it. Location labels can carry a small `LOC` marker; things dominate and need no marker. *(Update 2026-09-26: shipped as a `Site` marker on location labels.)*
...
**3. Let bootstrap claim `system` and `operator`, and skip the reserved-word list.** *(Amended: not what shipped. Bootstrap now derives both codes from `--org` / `--operator-org` the same way every other organization gets one, falling back to `system` / `operator` only when the name yields nothing valid. Pinning the two strings unconditionally meant bootstrap and the backfill migration disagreed about the same organization — a fresh install of `--operator-org "816tech"` produced `operator` while the migration produced `816tech`, so one deployment had two possible codes decided by install history, for a value that is immutable and printed on labels. Consequently these two strings are no longer reserved, which is acceptable because nothing resolves an organization by them.)* These two names want reserving, matching the `is_system_org` / `is_operator_org` special-case the org hooks already carry — but no list is needed to do it. Bootstrap creates both organizations, so if it sets their codes explicitly the global unique index makes those strings unavailable to everyone else for free. A reserved-word check would be a second mechanism enforcing what the constraint already enforces, and a second place to forget to update.
...
*(Update 2026-09-26: the rule quoted above is the one in force when this was written, and it was later found unsafe. Both sides of `organization = @request.auth.current_organization` default to the empty string, so a record with a blank organization matched any caller with a blank context. Every inventory read rule now also requires a non-blank context and a correlated membership in that organization. The conclusion stands — an org-scoped user credential still pulls inventory with nothing new — but copy the rule from `schema.json`, not from here.)*
...
Label generation shipped in both apps as the same component, provider-branded — the brand is on the sticker because whoever finds a broken device needs to know who services it, while the *tenant* name deliberately is not, since a sticker in a public hallway is readable by anyone walking past. *(Update 2026-09-26: both halves reversed in the platform. A label now prints the **organization's code** on its top line — its name only if it has no code — and no provider brand. A thing code is unique only within its organization, so the org code is the half that makes a sticker unambiguous to a technician servicing several customers, and it is immutable where a name is not. The brand is a deployment-wide setting that says nothing true about who owns a particular device, and a device already sits on its owner's premises, so naming the owner tells a passer-by very little. The QR payload is unchanged: still the bare record code.)* The scanner shipped in the helpdesk only; the platform already had a generic dashboard scanner that resolves a scanned value through a configured filter, and a bare code works with it unmodified — which is the clearest evidence available that the payload decision matched what a real consumer already expected.

One constraint surfaced only once the labels were real, and it is worth recording because nothing above anticipated it. **A label has a physical size, and the first cut did not.** Artwork laid out in CSS pixels printed at roughly 4.7″ × 1.7″ — matching no stock anyone sells. Labels are now dimensioned in **millimetres** at 2″ × 1″ and 4″ × 2″, with the per-size `@page` box written from script (`@page` cannot be interpolated from a template or a scoped style block, so a template-driven layout silently prints onto whatever paper the browser has selected).

*(Update 2026-09-26: this passage originally described a centred UHF RFID inlay keep-out band that both sizes reserved. It was removed. The band cost the small label a third of its text column to accommodate media the platform cannot use — there is no encoder, no reader, and no field on a thing for an EPC — and a 2″ × 1″ UHF label barely exists, since a UHF dipole needs length. Both sizes are plain thermal stock. If RFID ever arrives it comes back measured against a real inlay's datasheet.)*

This interacts with the density finding above rather than sitting beside it. At a 4-module quiet zone the worst-case realistic code is 33 modules across, giving 0.61 mm per module on the small stock (and 1.21 mm on the large) — above the ~0.5 mm a phone camera needs, but not by much on the small one. The URL form's 41×41 symbol would have pushed the small size below that floor, so on 2″ × 1″ stock the bare payload is not merely a better choice, it is the difference between a label that scans and one that does not.

Those module sizes are the check that the whole payload argument holds physically. *(Update 2026-09-26: an earlier revision of this section repeated the sizing paragraph with different figures — 0.69 / 1.38 mm. The figures above are the ones the current component is checked against.)* The URL payload would not have fit the small stock at all.
