...
1. **`organizations` gains a `code`** — globally unique, immutable, `^[a-z0-9][a-z0-9-]{1,30}$`, assigned at organization creation. *(Amended: the pattern originally required a leading letter. It was relaxed to allow a leading digit when an operator organization named `816tech` could not be migrated, and nothing downstream — NATS subject tokens, `edge-` JetStream domains, KV bucket names, RFC 1123 hostname labels — justified the restriction.)* This is the **only** globally unique identifier in the ecosystem. Everything below it stays unique within its organization, which is already enforced.
